...
func (q *Queries) GetAcount(ctx context.Context, id int64) (Account, error) {
	row := q.queryRow(ctx, q.getAcountStmt, getAcount, id)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.Owner,
...
const getAcountForUdpate = `-- name: GetAcountForUdpate :one
SELECT id, owner, balance, currency, created_at FROM accounts
WHERE id = $1 LIMIT 1
FOR NO KEY UPDATE
`

func (q *Queries) GetAcountForUdpate(ctx context.Context, id int64) (Account, error) {
	row := q.queryRow(ctx, q.getAcountForUdpateStmt, getAcountForUdpate, id)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.Owner,
